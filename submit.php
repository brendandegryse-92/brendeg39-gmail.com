...
elseif ($data[$arraylen]->tableName == "Fields") {
  if ($data[$arraylen]->FieldName == null && $data[$arraylen]->FarmName == "" && $data[$arraylen]->Acres == "" && $data[$arraylen]->FSA_Farm == "") {
 ...
 if ($data[$arraylen]->counter < $data[$arraylen]->length-1) {
$sql = "UPDATE fields SET FarmName = ?, FieldName = ?, Acres = ?, FSA_Farm = ?, FSA_Tract = ?, FSA_Field = ?, FSA_Area = ?, InsuranceID = ?, County = ?, Township = ?, FarmRange = ?, Section = ?, Legal = ?, Watershed = ?, Restriction = ?, Slope = ?, TRating = ?, Location = ?, PID = ?, TicketTrackID = ?, AutoSteerHeading = ?, IsActive = ? WHERE ID = ? AND UserID = ?";
$stmt = $connection->prepare($sql);
$stmt->execute([$data[$arraylen]->FarmName, $data[$arraylen]->FieldName, $data[$arraylen]->Acres, $data[$arraylen]->FSA_Farm, $data[$arraylen]->FSA_Tract, $data[$arraylen]->FSA_Field, $data[$arraylen]->FSA_Area, $data[$arraylen]->InsuranceID, $data[$arraylen]->County, $data[$arraylen]->Township, $data[$arraylen]->FarmRange, $data[$arraylen]->Section, $data[$arraylen]->Legal, $data[$arraylen]->Watershed, $data[$arraylen]->Restriction, $data[$arraylen]->Slope, $data[$arraylen]->TRating, $data[$arraylen]->Location, $data[$arraylen]->PID, $data[$arraylen]->TicketTrackID, $data[$arraylen]->AutoSteerHeading, $data[$arraylen]->Active, $_SESSION['rowPrimaryID'][$data[$arraylen]->counter], $_SESSION['ID']]);}
if (($data[$arraylen]->counter == $data[$arraylen]->length-1) && ($data[$arraylen]->FieldName != "")) {
